...
本文主要介绍了在使用 Python 面向对象编程时，如何使用上下文管理器和 with 语句替换异常处理中 try...finally 语句，并介绍了 with 语句的优点和应用场景。
...
expression是一个返回遵循上下文管理协议的对象的表达式。这个对象必须实现以下两个方法:
1. enter(): 在进入with语句块时被调用，返回一个对象供as子句使用。
2. exit(exc_type, exc_value, traceback): 在退出with语句块时被调用，用于执行清理操作。

在异常处理中，with...语句相当于 try-finally 语句的简写，可以替代 try-finally 的功能。一个很好的例子是文件处理，你需要获取一个文件句柄，从文件中读取数据，然后关闭文件句柄。如果不用 with 语句，代码如下：
...
在这个例子中, open()函数返回一个文件对象, 它实现了上下文管理协议。with语句会自动调用文件对象的enter()方法返回文件对象, 并在语句块结束时自动调用exit()方法关闭文件。这样可以确保文件在使用完毕后被正确关闭, 避免了资源泄露的问题。

使用 `with` 语句的优点包括:

1. 简洁和易读性:与手动管理资源的方式相比,`with` 语句使代码更加简洁,更容易理解。
2. 异常安全:即使在代码块中出现异常,资源也能得到正确释放,避免了资源泄露。
3. 可重用性:可以创建自定义的上下文管理器,封装资源的获取和释放逻辑。

![image](https://img2024.cnblogs.com/blog/2591203/202407/2591203-20240729002320601-131353361.png)
